...
async function loadPets() {
  ...
  try {
    ...
    console.log(err.message);
    if (listBox) listBox.innerHTML = `<div class="item">Não foi possível carregar pets.</div>`;
  }
}

function renderPets(pets) {
  const box = byId("petList");
  if (!box) return;
  ...
  pets?.forEach((pet) => {
    ...
    div.innerHTML = `
      ${photo ? `<img src="${photo}" alt="${pet.name || "Pet"}">` : `<div class="pet-mini-placeholder">🐾</div>`}
      <div class="pet-mini-info">
        <div class="pet-mini-title">${pet.name || "Pet sem nome"}</div>
        <div class="pet-mini-meta">${pet.breed || "Raça não informada"} • ${pet.size || "porte"}</div>
      </div>
    `;
    box.appendChild(div);
  });
